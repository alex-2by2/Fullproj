...
class FileEntry {
  final String name;
  final String uri; // content://... or file:///...
...
// Simple in-memory thumbnail cache (URI string -> bytes)
...
class _StatusHomePageState extends State<StatusHomePage> {
          ...
          Padding(
            ...
            child: Row(
              children: [
                ...
                const SizedBox(width: 8),
                ElevatedButton(
                  onPressed: entries.isEmpty ? null : _deleteSelected,
  ...
  // selection tracking
  final Set<String> _selectedUris = {}; // content uri strings
  ...
  Future<void> _pickFolder() async {
    try {
      ...
      if (!kIsWeb && Platform.isAndroid) {
        ...
        treeUri = res;
        // Automatically list files after pick
        await _listFiles();
      } else {
        // Fallback for other platforms: use FilePicker directory selection
        ...
        treeUri = path;
        // On non-Android we can list local files normally
  ...
  Future<void> _listFiles() async {
    ...
    try {
      ...
      for (final item in res) {
        if (item is Map) {
          list.add(FileEntry.fromMap(item));
